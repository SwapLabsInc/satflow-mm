...
class OrdinalsCollectionManager extends BaseCollectionManager {
  ...
  async processCollection(collectionId, walletItems, biddingAddress, biddingBalance) {
    ...
    myListings.forEach(listing => {
      if (!listingsMap.has(listing.inscriptionId)) {
        listingsMap.set(listing.inscriptionId, []);
      }
      listingsMap.get(listing.inscriptionId).push(listing);
    });
    ...
    for (const item of collectionItems) {
      try {
        ...
        const { price: satflowPrice, isUndercut: isSatflowUndercut } = calculateDynamicPrice(targetListPrice, satflowListings);
        let finalSatflowPrice = satflowPrice;
